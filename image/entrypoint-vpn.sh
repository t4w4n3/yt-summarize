...
#
# Sans config (clone frais): message clair + arrêt propre (exit 0, donc pas de
# boucle de restart avec restart: on-failure). Le stack tourne quand même.

if [ ! -f /mullvad/wg0.conf ]; then
  echo "Mullvad non configuré — lance 'mise run mullvad init' (README §Mullvad VPN)." >&2
  echo "Le service vpn reste arrêté; pour télécharger en direct: MULLVAD_ENABLED=false dans .env" >&2
  exit 0
fi
