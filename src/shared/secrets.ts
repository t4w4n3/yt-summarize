 ...
 * Placeholder handling (sk-or-missing, # empty) lives here exactly once.
 */
export function resolveYouTubeCookiesPath(): string | null {
  if (fs.existsSync(COOKIES_SECRET_PATH)) {
    try {
      ...
      if (stat.size > 0) {
        const content = fs.readFileSync(COOKIES_SECRET_PATH, 'utf8');
 ...
 * Throws a plain Error when the podman secret exists but contains an invalid
 * non-placeholder value, or when GPG decryption fails — callers (worker layer)
 * wrap this into a StageError so `shared` never depends on `worker`.
 */
  ...
  if (fs.existsSync(OPENROUTER_SECRET_PATH)) {
    try {
      ...
      if (!key) {
        ...
        throw new Error(`secret at ${OPENROUTER_SECRET_PATH} did not contain a valid sk-or- key`);
      }
    } catch (error) {
      if (error instanceof Error && error.message.includes('did not contain')) throw error;
...
function decryptViaGpg(gpgPath: string): Promise<string> {
  return new Promise<string>((resolve, reject) => {
    ...
    child.on('error', (error: Error) => {
      ...
      reject(new Error(`Could not decrypt the OpenRouter key: ${error.message}`));
    });
    child.on('close', (code) => {
      clearTimeout(timeout);
      const key =
      ...
      if (code !== 0 || !key.startsWith('sk-or-')) {
        reject(new Error(`Could not resolve the OpenRouter credential. ${stderr.trim().slice(-400)}`));
      } else {
        resolve(key);
      }
    });
